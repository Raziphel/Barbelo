...
class leaderboard(Cog):
    ...
    async def five_minute_loop(self):
        ...
        for index, (user, rank) in enumerate(zip(users, ranks)):
            if index < 10:
                text.append(f"#{index+1} **{user.name}** ─── {rank.coins:,}{self.bot.config['currency_emoji']['coin_emoji']}")
            else:
                text2.append(f"#{index+1} **{user.name}** ─── {rank.coins:,}{self.bot.config['currency_emoji']['coin_emoji']}")
