...
class leaderboard(Cog):
    ...
    async def five_minute_loop(self):
        ...
        for index, (user, rank) in enumerate(zip(users, ranks)):
            if index < 10:
                text.append(f"#{index+1} **{user.name}** ─── {self.bot.config['emojis']['coin_emoji']}{floor(rank.coins):,}x")
            else:
                text2.append(f"#{index+1} **{user.name}** ─── {self.bot.config['emojis']['coin_emoji']}{floor(rank.coins):,}x")
