...
class store_Handler(Cog):
    ...
    async def store_msg(self):
        ...
        embed1.add_field(name=f"⊰ ✨ Discord Nitro ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}10x**\n\n```Get the 10$ Discord Nitro!```", inline=True)
        embed1.add_field(name=f"⊰ 💸 Get 5$USD! ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}5x**\n\n```Turn all your hellstones into $USD!```", inline=True)

        embed2=Embed(description=f"# Permissions\n`All these listed items give you general permissions on the server!`", color=0x00FF00)
        embed2.add_field(name=f"⊰ 📚 Library Pass ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}25x**\n\n```Get access to all of the server's logs!```", inline=True)
        embed2.add_field(name=f"⊰ 🎫 Image Pass ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}25x**\n\n```Get permission for images & embeds in General Chats.```", inline=True)
        embed2.add_field(name=f"⊰ 🔊 SoundBoard Pass ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}25x**\n\n```Get access to using the soundboard in VC!```", inline=True)
        embed2.add_field(name=f"⊰ 🎁 Stats Channel ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}75x**\n\n```Get permission to the Stats Channels!```", inline=True)
        embed2.add_field(name=f"⊰ 🧶 Thread Perms ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}75x**\n\n```Get perms to create threads!```", inline=True)
        embed2.add_field(name=f"⊰ 🔮 External Emotes ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}75x**\n\n```Get access to using your external emotes and stickers!```", inline=True)
        ...
        embed3.add_field(name=f"⊰ 🧤 Thievery ⊱", value=f"**╰⊰ 50 {self.bot.config['currency_emoji']['coin_emoji']}x**\n\n```Gain the ability steal from others!```", inline=True) 
        ...
        embed4.add_field(name=f"⊰ 🌻 Sun Flower ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}1x**\n\n<@&{self.bot.config['color_roles']['sun_flower']}>", inline=True)
        embed4.add_field(name=f"⊰ 🥗 Nephritis ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}1x**\n\n<@&{self.bot.config['color_roles']['nephritis']}>", inline=True)
        embed4.add_field(name=f"⊰ 🧿 Belize Hole ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}1x**\n\n<@&{self.bot.config['color_roles']['belize_hole']}>", inline=True)
        embed4.add_field(name=f"⊰ 🪀 Wisteria ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}1x**\n\n<@&{self.bot.config['color_roles']['wisteria']}>", inline=True)
        embed4.add_field(name=f"⊰ ⛅ Clouds ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}1x**\n\n<@&{self.bot.config['color_roles']['clouds']}>", inline=True)
        embed4.add_field(name=f"⊰ 🧩 Turquoise ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}1x**\n\n<@&{self.bot.config['color_roles']['turquoise']}>", inline=True)

        embed4.add_field(name=f"⊰ 🍑 Pastel Pink ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}3x**\n\n<@&{self.bot.config['color_roles']['pastel_pink']}>", inline=True)
        embed4.add_field(name=f"⊰ 🍇 Pastel Violet ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}3x**\n\n<@&{self.bot.config['color_roles']['pastel_violet']}>", inline=True)
        embed4.add_field(name=f"⊰ 🧊 Pastel Blue ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}3x**\n\n<@&{self.bot.config['color_roles']['pastel_blue']}>", inline=True)
        embed4.add_field(name=f"⊰ 🥒 Pastel Green ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}3x**\n\n<@&{self.bot.config['color_roles']['pastel_green']}>", inline=True)
        embed4.add_field(name=f"⊰ 🍋 Pastel Yellow ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}3x**\n\n<@&{self.bot.config['color_roles']['pastel_yellow']}>", inline=True)
        embed4.add_field(name=f"⊰ 🍓 Pastel Red ⊱", value=f"**╰⊰ {self.bot.config['currency_emoji']['coin_emoji']}3x**\n\n<@&{self.bot.config['color_roles']['pastel_red']}>", inline=True)
